...
// Helper to select a project from the dropdown
...
describe("PullRequests", () => {
	...
	it("should display projects when available", () => {
		...
		// Project 2 has no repository, so it shows "(no repo)" suffix
	...
	it("should not auto-select when multiple projects exist", () => {
		mockUseQuery.mockImplementation((query) => {
			...
			if (query === mockApi.github.getPRsForProject) {
				// Return undefined for skipped query
				return undefined;
			}
			return undefined;
		});
		...
		// Verify PR query is skipped (called with "skip" since no project is selected)
